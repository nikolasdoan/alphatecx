...
## [2026-09-05] ops+fix | Deploy key unfreezes the nightly dashboard push
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, infrastructure-accounts]
- The nightly `chore(graph): daily refresh [skip ci]` push to `main` had been rejected since 2026-08-16 (`GH013: Changes must be made through a pull request`), so the console's flow dashboard, ticker pages and graph view served frozen content for three weeks.
- **The finding that decided the approach: there is NO ruleset bypass actor that covers `github-actions[bot]` pushing with the default `GITHUB_TOKEN`.** The bypass picker offers Roles / Teams / Apps / Users / Others; searching it for "GitHub" returns only "GitHub Merge Queue". So "just add Actions to the bypass list" — the obvious first answer, and the one I gave — is not actually available.
- **The auto-merge alternative fails for a related reason** and was dropped: a PR opened with `GITHUB_TOKEN` does not trigger `pull_request` workflows, so the required `test` check would never run and auto-merge would never fire. That is the same rule that stopped CI on PR #16. Both routes therefore need a credential; the question is only which.
- **Chosen: a write-enabled deploy key**, added to the ruleset's Bypass list (Others → Deploy keys) and to `actions/checkout` as `ssh-key`. It is the narrowest actor that works — write to this repository only, no user identity, cannot open pull requests, cannot reach anything else in the org. A fine-grained PAT would have acted as [niko] and been strictly broader.
- Key generated on [niko]'s own machine and moved via `pbcopy` straight into the deploy key and the `DASHBOARD_DEPLOY_KEY` secret, then deleted locally — never through a terminal echo or a chat paste. That is the rule this project earned the hard way twice (Neon owner DSN, 2026-08-09; Zeabur owner DSN, 2026-08-12).
- **The "Dashboard frozen" warning is KEPT, not removed.** Getting it working once is not the same as noticing when it stops — the annotation now names `DASHBOARD_DEPLOY_KEY` and the bypass entry, so a rotated key or a rewritten ruleset reports itself instead of silently re-freezing the console behind a green run. Pinned by tests: deleting the `ssh-key` line fails 2, flipping `persist-credentials` to false fails 1.
- `checkout` falls back to `GITHUB_TOKEN` when the secret is empty, so a fork degrades to the old behaviour (push fails, warning fires) rather than the job dying.
- 4 new tests; suite 789 pass, ruff clean. Unverified until Monday's scheduled run — the first real push is the acceptance test.
## [2026-09-14] decision | marketecx: alphatecx is the side with no 統一編號
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server]
- [niko] is joining bizmap (255,637 Taipei business registrations, geocoded), this repo and tecxwork into **marketecx**, reached through a queryable Claude connector — "ask what restaurants are on 永康街, query both Google Maps and our data, compare". All three codebases read; design written up in `tecxmate/bizmap` → `docs/marketecx.md` ([PR #32](https://github.com/tecxmate/bizmap/pull/32)). **Nothing changed in this repo.**
- **The finding: there is no 統一編號 anywhere in alphatecx.** `dim_ticker` has only `ticker_id`. bizmap's `company.id` *is* the 統編 for every registration and tecxwork has `clients.unified_business_no`, so this is the side that cannot be joined. One column on `dim_ticker` from MOPS 基本資料 closes it. Without it the join is Chinese company-name matching — with 股份有限公司 suffixes, no uniqueness, and issuer renames, which already broke `view_ticker_momentum` once.
- **Nothing may pull from this Postgres, and that decides the architecture.** TLS is disabled outright here (`sslmode=require` rejected), tolerable only across Zeabur's private network; a Cloudflare Worker at the edge is not on it. So marketecx centralizes into D1 as a **read model** and the nightly Actions job — which already holds the credentials — *pushes* an extract over HTTPS. Nothing pulls.
- What ships is the **pre-computed views**, not the maths: `q_pca_decompose`, `q_cointegration_pair` and `q_factor_alpha` are numpy and SQLite cannot run them. The MCP tools already read views rather than computing per request, which is exactly the shape that exports.
- **This repo's connector is the template for the new one** — `_stamp()` provenance, the `sc_capabilities` registry with the test that pins it in both directions, `query_safety.safe_flow_col`, Asia/Taipei dates, tool descriptions as the interface. The one thing not to copy is URL-as-secret auth: bizmap already has OAuth, sessions and an atomic per-day quota, and an LLM issues 400 queries where a human issues four.
- created [topics/marketecx.md](topics/marketecx.md); index updated.

## [2026-09-14] decision | The 統編 bridge is specified, and the source needs no name matching
attributed_to: [niko, claude-agent]   belongs_to: [marketecx, system-architecture]
- [niko] stated bizmap's requirement as a spec: one table, ~2,000 rows, `統一編號 → ticker → 市場別 → company name`, CSV or JSON; only the 統編 join key essential. Two conditions — a per-row confidence flag if the mapping is name-matched, and government provenance under 政府資料開放授權條款, because a vendor feed would muddy bizmap's central claim and it would drop the facet rather than accept that.
- **Both are satisfiable and the first largely dissolves: the government file carries 統一編號 in the same row as 公司代號.** `mopsfin.twse.com.tw/opendata/t187ap03_L.csv` (上市, [dataset 18419](https://data.gov.tw/dataset/18419)) and `t187ap03_O.csv` (上櫃, [25036](https://data.gov.tw/dataset/25036)), published by 金管會證期局 via TWSE under 政府資料開放授權條款第1版. No matching step, no vendor, no key — and the same licence bizmap already carries, so its attribution block gains a line and nothing else changes.
- The `match` field is kept anyway, `"exact"` on every row: it lets bizmap publish on `match == "exact"` as asked without the column changing meaning if a fuzzy source is ever added, and it makes a future degradation visible rather than silent.
- **Two things deliberately not claimed.** 興櫃 is unverified — 上市 and 上櫃 are confirmed, and if no equivalent dataset exists then `市場別` carries two of three values. And the column list comes from two independent secondary sources, because `mopsfin.twse.com.tw` and `data.gov.tw` are both refused by this environment's egress proxy (403 on CONNECT); the file was never opened here. So the harvester asserts the header on every run and fails loudly rather than trusting it — the discipline `apply_delta.py` already applies to grants.
...
## [2026-09-14] ingest | The 統編 bridge was built — 2,340 rows, and 興櫃 exists after all
attributed_to: [niko]   belongs_to: [marketecx, system-architecture]
- [niko] ran `python -m src.harvester.tax_ids` on a machine that can reach TWSE and delivered the table to bizmap as `pipeline/listed_companies.csv` ([bizmap#34](https://github.com/tecxmate/bizmap/pull/34)). **2,340 rows**, `match: exact` on every one — the join came from the 統一編號 column, not from name matching.
- **Both open items in the spec are resolved.** 興櫃 *does* have an equivalent open dataset — `t187ap03_R`, 363 rows — so `市場別` carries all three values: 上市 1,086 · 上櫃 891 · 興櫃 363. And the column list taken from secondary sources was right.
- Validated against the delivered file rather than its commit message: 7 fields on every row, every `tax_id` eight digits, **zero duplicate `tax_id` and zero duplicate `ticker_id`**, `source` splitting 1,086/891/363 across the three files.
...
## [2026-09-16] feat | stock.tecxmate.com gets a public face — infrastructure-framed, not a product page
attributed_to: [niko, claude-agent]   belongs_to: [marketecx, system-architecture]
- [niko] asked for a landing page and an analysis page at `stock.tecxmate.com` showing what the system can do. **The framing was the decision, not the design.** [2026-09-15-alphatecx-feeds-bizmap-not-sold](decisions/2026-09-15-alphatecx-feeds-bizmap-not-sold.md) had just established that the PRIVATE framing is what closes the investment-advice licensing gate — so a public page marketing Taiwan stock analysis would have reopened, the day after, the gate that decision closed. Asked; [niko] chose **public but infrastructure-framed**.
- What that means concretely, and it is a content rule rather than a tone: the pages describe the PIPELINE — coverage, cadence, provenance, licensing — and contain **no ticker, no screen, no ranking, and nothing readable as a view on any security**. The "analysis" page analyses the DATA, not the market.
- `web/app/page.tsx` is now the landing page; the chat moved to `web/app/chat/page.tsx` **unchanged** — only the route. `/` had been the assistant, and a landing page that is not at `/` is not a landing page.
- `web/app/coverage/page.tsx` carries the three things worth publishing: the dataset table (what, how much, how often), the **when-knowable map** (7 of 11 macro series close before Taipei opens; Tokyo/Seoul/Shanghai/Hong Kong trade ALONGSIDE it), and the method constraints — baseline, 0.585% round-trip cost, date-clustered effective n, next-close entry, uncorrected survivorship. Publishing the limits is the point: each one, left out, flatters a result, and all five bias the same direction.
- Every figure is read off the codebase rather than estimated (53 tools, 11 series / 7 markets, 16 feeds, 2,340 companies · 上市 1,086 · 上櫃 891 · 興櫃 363). A comment at the top of each constant says so, because a number on a public page that drifts from the system is the same failure as a stale tutorial — and unlike `docs/TUTORIAL.md`, **these are NOT generated and nothing fails when they rot.** That is a known gap, not an oversight.
- **Found while working, not fixed here:** `web/` has no middleware, so the chat at that domain is open to anyone who reaches it — and it holds the MCP token server-side and spends the Claude API key. Moving it off `/` reduces the odds of a passer-by landing in it, but does not gate it. Raised for [niko] as its own decision.
- Verified by building and serving: `/`, `/chat`, `/coverage` all 200 and prerender static; biome clean on the three new files (the repo's other 95 biome errors are pre-existing and untouched). Screenshots reviewed before shipping — one stat card was re-worded because it wrapped and broke the row's alignment.

## [2026-09-16] security | The chat is gated — and the API routes are the reason, not the page
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, marketecx]
- Found while building the public landing: `web/` had **no middleware at all**, so the assistant at `stock.tecxmate.com` was open to anyone who reached the domain. [niko]: "gate the chat."
- **Gating the page alone would have been theatre.** `/api/chat` is a general-purpose proxy to the MCP server — it holds the bearer token server-side, reaches all 53 tools including the ones that WRITE, and spends the Anthropic API key per message. Anyone who could call that route directly never needed the page. So the gate covers `/chat/*` **and** `/api/*`.
- **The matcher names what is gated, rather than excluding what is not.** `matcher: ["/chat/:path*", "/api/:path*"]` leaves `/` and `/coverage` public *by construction*. The inverted form — gate everything except a pattern — silently starts gating a new public page or silently stops gating a new API route depending on which way the regex is wrong, and both failures are quiet. Both public pages are fully static and call no API, so nothing on them broke.
- **Fails CLOSED.** An unset `CHAT_PASSWORD` returns 503, not 200: a gate that looks installed and is not is strictly worse than no gate, because nobody goes back to check. 503 rather than 401 because retrying with a password cannot help — the deployment is misconfigured, and the body names the variable to set. The landing page stays up either way.
- Constant-time comparison written out by hand: `node:crypto.timingSafeEqual` does not exist in the edge runtime the middleware runs in, and `===` returns on the first differing byte. Length is folded into the result rather than short-circuited on, and the user check is not `&&`-ed ahead of the password check for the same reason.
- Basic auth over a login form on purpose: a shared credential for a handful of people, no session to store, expire or invalidate wrongly. **If per-person identity is ever wanted, the answer is Cloudflare Access on the hostname** — runbook already at [`console-auth`](topics/console-auth.md) — not a larger version of this file.
- Verified by serving the real build: unauthenticated `/` and `/coverage` 200; `/chat`, `/api/chat`, `/api/watchlist` 401; correct credentials 200; wrong password 401; secret unset 503 with the landing still 200.
- **Action required before this helps:** set `CHAT_PASSWORD` in the Vercel project. Until it is set the chat is closed rather than open, which is the correct direction to fail but is still a broken chat.

## [2026-09-16] product | The public surface leads with structure — /market-map ships three interactive graphs
attributed_to: [niko, claude-agent]   belongs_to: [marketecx, system-architecture]
- [niko]: *"make interactive graphs that people care about the market want to see, search it first, show some data that other 三竹股市 cant show, i dont want to see all texts and numbers."* Searched first, as asked.
- **What the research said.** 三竹股市 is criticised for standing still ("該有的功能都有，但不會讓人期待有任何進步") and has no relationship view at all. 三大法人買賣超 is already visualised by TWSE, TPEx, HiStock, 玩股網 and 富邦 — another leaderboard adds nothing. Supply-chain data exists only as **directories** (財報狗 per-industry 上下游類股, TPEx 產業價值鏈資訊平台): they answer "what is 3017 in?" one company at a time, as a list. **Nobody draws the graph**, and nobody publishes a correlation embedding or a session-timing map.
- Shipped `/market-map`: (1) the supply chain laid out by longest upstream path, so column position *is* chain depth, with transitive hover; (2) the 120-day correlation MDS embedding with supply edges and ρ ≥ 0.70 edges as toggles; (3) a 24-hour market clock drawn from `when_known`.
- **Crossing (1) and (2) produced the actual finding: 36 declared supply relationships, 15 pairs at ρ ≥ 0.70, overlap ZERO.** Every tight pair is same-sub-industry — 華邦電/南亞科 記憶體, 欣興/景碩 載板, 士電/華城 重電, 鴻海/緯創/英業達 伺服器代工 — and none is up/downstream. `corr_edges` is not a top-N list (the snapshot emits *every* classified pair above the floor), so this is about the whole universe, not a leaderboard cut-off. The page states the reading — 「A 供貨給 B」不等於「A 和 B 會一起漲」 — and names what it cannot tell apart: a common driver versus our own classification being incomplete.
- **Scope held to the "public but infrastructure-framed" line.** Topology and past co-movement are descriptions; a ranking or a screen would be a recommendation. No flow leaderboard, no screen, no signal — and the page says so in its own 「這頁不做的事」 block.
- Red is up and green is down, per 台股 convention rather than TradingView's. The audience is Taiwanese; consistency with the West would be a legibility bug here.
- **Data is a committed copy, not a fetch.** The console's snapshot is behind the URL-secret gate, and a proxy route would need a Vercel env var set before the page worked — `CHAT_PASSWORD` is already unset and closing the chat, so a second one was not worth it. The nightly job now re-syncs the copy between the snapshot regenerate and the push, so both land in one commit; `tests/test_web_market_map.py` fails if they ever differ.
- **`[skip ci]` on the nightly commit is load-bearing in two directions.** It stops the deploy-key push re-triggering Actions (unlike `GITHUB_TOKEN`, a deploy-key push *does* trigger workflows) — and Vercel honours the same marker, so the public page will not rebuild on its own. Optional `VERCEL_DEPLOY_HOOK` secret closes that loop; unset, the map refreshes on the next deploy and says how old its data is.
- **New test pins the clock to `src/harvester/macro.py`** — every market drawn, no market invented, `knownBeforeOpen` equal to `when_known` series by series. It is the page's only claim about the world rather than about our data, and disagreeing with the harvester would publish the exact look-ahead bias `when_known` exists to prevent. Writing it surfaced that FX was missing from the clock (6 of 7 markets), now drawn as a full 24h ring.
- **`web/biome.json` added** — there was no biome config at all, so `pnpm lint` walked `.next/` and reported ~12,500 errors from build output. A lint gate nobody can read is a lint gate nobody runs.
- Verified against a real production build: static prerender, zero horizontal overflow at 390 px, biome clean, `ruff check .` clean, 814 tests (was 802). Every section screenshotted and fixed across three rounds — arrowheads were landing under the labels they pointed at, correlation labels overlapped because separation was radial rather than box-shaped, and the clock's caption sat under its own hand.

## [2026-09-16] ops | The research terminal is switched off — declared, not inferred
attributed_to: [niko, claude-agent]   belongs_to: [marketecx, system-architecture]
- [niko]: *"turn off the chat for now."* It was already unreachable — but only because `CHAT_PASSWORD` had never been set on Vercel, so the fail-closed branch 503'd everyone.
- **Unreachable by accident and unreachable on purpose looked identical**, which is the defect this repo already has written down once: "disabling" Telegram by unsetting `TELEGRAM_TOKEN` made a dead notify path indistinguishable from a quiet one, and every alert sat at `pushed:false` for weeks behind it. CLAUDE.md says **do not disable by unsetting the secret**. Same fix applied here rather than leaving the accident in place and calling it the feature.
- **`CHAT_ENABLED`, defaulting to OFF.** Compared against the exact string `"true"` — not `!== "false"`, not truthiness — so a typo or a half-configured environment leaves the terminal closed rather than open. Default-off also means the switch needed **no action on Vercel** to take effect, which matters because the variable that was supposed to be set there is precisely the one nobody set.
- **Off returns 404, not 401 or 503.** Both of those advertise the surface: 401 says "guess the password", 503 says "try later". Off should look like nothing is there, and 404 leaks nothing about what the route would have reached. The misconfiguration 503 (enabled, no password) stays as a *separate* state with its own message, so "closed" and "broken" are still distinguishable from the outside.
- **Turning it back on is two deliberate variables** — `CHAT_ENABLED=true` **and** `CHAT_PASSWORD` — not one forgotten one.
- The landing page reads the same flag, so the nav link and the route turn off together. A link to a 404 reads as a broken site rather than a closed surface. Read at build time, which cannot drift *within* a deployment — and flipping the variable needs a redeploy anyway.
- `/`, `/coverage` and `/market-map` are untouched and still linked: they are fully static and call no API route, so none of this is on their path.
- `tests/test_web_chat_gate.py` (8 tests) pins the switch: flag read, default off, checked **before** the password branch, 404 not 401/503, both `/chat/*` and `/api/*` still in the matcher when it is back on, matcher still not inverted, and the public links outside the guard.
- Verified against real builds in all four states: **off** (chat + every API route 404, public pages 200, link hidden) · **on + password** (401 unauth, 200 with creds, 401 wrong password) · **on + no password** (503 with the "set this variable" message) · public pages 200 throughout.

## [2026-09-16] product | The chain map gains the two zones outside Taiwan — and three different provenances
attributed_to: [niko, claude-agent]   belongs_to: [marketecx, supply-chain-audit-2026-05-10]
- [niko]: *"add 終端客戶 as blocks, but put it outside the taiwan rectangle, same with upstream suppliers for semiconductor, chemical and equipment from Japan US Netherlands ASML."*
- **The chain does not start or end in Taiwan, and a map that stops at the coastline implies it does.** `/market-map` §01 now draws three zones: 國外上游 (left), a labelled 台灣上市櫃 rectangle (middle), 終端客戶 (right). Upstream the tools and the chemistry are a handful of foreign firms — one of them, ASML, with no competitor at all; downstream essentially the whole AI build is bought by a dozen American names. Both facts explain more about a move in 台積電 than anything inside the box.
- **The three zones have three different provenances, so they are drawn three different ways** — the page says this out loud, because it is the whole basis for trusting the other graphs:
  - **Middle** = our data. Prices, institutional flow, the 120-day correlation everything else is computed from.
  - **Right** = DERIVED from the classification's own `partners` field, normalised here. Real edges; we just hold no prices for the customers. A firm needs ≥ 2 Taiwan companies naming it to get a block — one mention is a single business relationship, and a wall of one-offs would bury the point.
  - **Left** = hand-written context, the only part of the page not read off the data. Drawn dashed and unfilled, and **carries no number anywhere** — a figure there would look exactly like the ones that are real.
- **One aggregate inflow arrow from the left band, not a line per firm.** We hold no edge data for the foreign upstream; drawing specific relationships would assert commercial facts the snapshot does not contain. The band says what flows in, not who sells to whom.
- **Customer links are drawn only for the current selection.** All ~120 at once cross the busiest part of the picture and hide the chain underneath. Hover a 台廠 → its customers light up; hover a customer → it runs backwards and lights every 台股 name that sells to it, which is the same question from the other end ("誰供貨給 NVIDIA?" — 14 of the 51).
- **`partners` is messier than it looks, and three filters exist because of it.** It mixes firms with end-uses (`auto`, `AI-PC`, `industrial`, `various`, `IDMs` — rendering those as customer blocks would invent buyers); it spells one firm two ways (`NVIDIA-via-PCB` records that the relationship is one hop removed, a fact about the EDGE, not a different customer); and it contains **Taiwanese** entities (`TSMC` is 2330, already a node; `TPC` is 台電) and **upstream** ones (ASML, Applied Materials, Lam, KLA — the firms our 廠務 and 設備代理 names work *with*).
- **A test caught a latent double-count that eyeballing did not.** `Arm` sits in the hand-written upstream band *and* in 2454 聯發科's `partners`. One mention is below the customer threshold, so nothing looked wrong today — it would have silently appeared on both sides of the box the moment a second name cited it. Arm licenses IP *in*, so it is upstream. `tests/test_web_chain_zones.py` (9 tests) pins the seams: no firm on both sides, no Taiwanese entity as a foreign block, no non-company rendered, aliases folded before counting, the threshold stated and ≥ 2, every curated customer actually present in the snapshot, and **no numeric field permitted anywhere in the hand-written band**.
- Also pins that `NOT_A_COMPANY` contains no tokens the snapshot has stopped emitting — a stale filter hides the next real one.
- Verified against a production build at 1500px and 390px: zero horizontal overflow, both hover directions correct, the card moved to bottom-left so it stops covering the customer band. 831 tests (was 822), ruff clean, biome clean.
